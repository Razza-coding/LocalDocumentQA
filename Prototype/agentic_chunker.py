...
'''
Orignal Code Developped by : "https://github.com/FullStackRetrieval-com/RetrievalTutorials/blob/main/tutorials/LevelsOfTextSplitting/agentic_chunker.py"

This version modify few section to fit in my system
 - Uses Ollama local llm instead of OpenAI
 - Uses self made create_extraction_chain_pydantic function for newer LangChain version
 - Creates a custom extraction class for Chunk ID bandaid, only extract 1 ID at a time
 - Create lenient prompt for local llm (not as smart as OpenAI), uses regex to clear noisy output.
 - Returns formated string when pretty print, for log writer
'''

load_dotenv()

class SchemaExtraction:
    def __init__(self, schema: BaseModel, llm, extraction_prompt: PromptTemplate):
        self.parser = PydanticOutputParser(pydantic_object=schema)
        self.extraction_prompt = extraction_prompt
        self.llm = llm
        self.runnable = self.extraction_prompt | self.llm | self.parser
        pass
    def run(self, input_text):
        #output_text = self.runnable.invoke({ "input" : input_text })
        output = []
        output.append(self.extraction_prompt.invoke({ "input" : input_text }))
        output.append(self.llm.invoke(output[-1]))
        json_blocks = re.findall(r'\{[^{}]*[^{}]*\}', output[-1].text(), re.DOTALL)
        output[-1].content = json_blocks[0]
        output.append(self.parser.invoke(output[-1]))
        return output[-1]

def create_extraction_chain_pydantic(pydantic_schema: BaseModel, llm):
    ''' extract 1 item form target '''
    items = pydantic_schema.__annotations__.keys()
    items = [str(i) for i in items]
    target_item = ''.join(items)
    empty_result = f"{{\"{target_item}\" : \"\"}}"
    extraction_prompt = PromptTemplate.from_template(
        (
            f'Verify is from the following text is a possible {target_item} or not, output them in Json format.\n'
            'Only return the Json result, do not include any explanations or additional text.\n'
            'If text does not contain something similar to Chunk IDs are found, return:\n'
            f'{{{empty_result}}}\n'
            'Text:\n'
            '{input}'
        )
    )
    return SchemaExtraction(pydantic_schema, llm, extraction_prompt)
...
class AgenticChunker:
    ...
    def pretty_print_chunks(self):
        ...
        for chunk_id, chunk in self.chunks.items():
            pretty_print_str += f"Chunk #{chunk['chunk_index']}\n"
            pretty_print_str += f"Chunk ID: {chunk_id}\n"
            pretty_print_str += f"Summary: {chunk['summary']}\n"
            pretty_print_str += f"Propositions:\n"
            for prop in chunk['propositions']:
                pretty_print_str += f"    -{prop}\n"
            pretty_print_str += "\n\n"
    ...
    def pretty_print_chunk_outline(self):
        ...
        pretty_print_chunk_outline += "Chunk Outline\n\n"
        pretty_print_chunk_outline += self.get_chunk_outline() + "\n"
        print(pretty_print_chunk_outline)
        return pretty_print_chunk_outline
